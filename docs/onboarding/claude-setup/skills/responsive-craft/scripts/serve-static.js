...
(function() {
  ...
  // Only reload on the file-change signal — the server also sends
  // 'connected' on initial handshake, which would otherwise cause a
  // tight reload loop (load → connect → handshake → reload → repeat).
  source.onmessage = function(e) { if (e.data === 'reload') location.reload(); };
...
function serve(port) {
  const server = http.createServer((req, res) => {
    ...
    // Strip query string before resolving to a file — `?url=...&breakpoints=...`
    // would otherwise become part of the literal path lookup and 404.
    const reqPath = req.url.split('?')[0];
    let filePath = path.resolve(rootDir, decodeURIComponent(reqPath).replace(/^\/+/, ''));
